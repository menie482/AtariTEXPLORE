...
for i in range(maxEpisodeReached):
  total = 0
  trialsThatReachedThisEpisode = 0
  for j in range(len(trials)):
    trial = trials[j]
    if episodesByTrial[j] > i:
      total = total + int(trial[i])
      trialsThatReachedThisEpisode = trialsThatReachedThisEpisode + 1
  totals.append((total, trialsThatReachedThisEpisode))

averages = []
for (total, numTrials) in totals:
  averages.append(total/numTrials)
print averages
