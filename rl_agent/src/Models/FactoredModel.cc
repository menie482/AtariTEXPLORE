...
bool FactoredModel::initMDPModel(int nfactors) {
    ...
    if (nModels == 1) {
        ...
        // assign terminal model
        modelType = modelSpecs[modelSpecs.size() - 1].modelType;
        if (episodic) {
            if (modelType == C45TREE && nModels == 1) {
                terminalModel = new C45Tree((id*(nfactors+1))+nfactors+1, treeBuildType,5, M, 0, rng);
            }
            else if ((modelType == M5MULTI || modelType == M5SINGLE) && nModels == 1) {
                terminalModel = new M5Tree((id * (nfactors+1)) + 1+nfactors, treeBuildType, 5, M, 0, simpleRegress, false, treeThresh, rng);
            }
            else if ((modelType == M5ALLMULTI || modelType == M5ALLSINGLE) && nModels == 1) {
                terminalModel = new M5Tree((id * (nfactors+1)) + 1+nfactors, treeBuildType, 5, M, 0, simpleRegress, true, treeThresh, rng);
            }
            else if ((modelType == LSTMULTI || modelType == LSTSINGLE) && nModels == 1) {
                terminalModel = new LinearSplitsTree((id * (nfactors+1)) + 1+nfactors, treeBuildType, 5, M, 0, simpleRegress, treeThresh, rng);
            }
            else if (modelType == STUMP && nModels == 1) {
                terminalModel = new Stump((id * (nfactors+1)) +1+ nfactors, 1, 5, M, 0, rng);
            }
            else if (predType == SEPARATE && nModels > 1) {
