		...
		else if (nModels > 1 || modelType == ALLM5TYPES)
		{
			...
			if (i == 0)
			{
				rewardModel = new MultipleClassifiers((id * (nfactors+1)) + nfactors,
													  modelSpecs[modelSpecs.size() - 2].modelType, predType,
													  nModels, treeBuildType, 5,
													  FEAT_PCT, // remove this pct of feats
													  EXP_PCT, treeThresh *rRange, stoch, rng);
				if (episodic)
				{
					terminalModel = new MultipleClassifiers((id * (nfactors+1)) +1+ nfactors,
															modelSpecs[modelSpecs.size() - 1].modelType, predType,
															nModels, treeBuildType, 5,
															FEAT_PCT, // remove this pct of feats
															EXP_PCT, treeThresh, stoch, rng);
				}
	...
	*/
	...
	if (!e.terminal)
	{
		for (unsigned i = 0; i < e.next.size(); i++)
		{
			// CHRIS-ADDED CODE
			// only gives specified dependencies to feature models
			std::vector<float> featuredInputs(modelSpecs[i].dependencies.size());
			for (unsigned j = 0; j < featuredInputs.size(); j++)
			{
				featuredInputs[j] = inputs[modelSpecs[i].dependencies[j]];
			}
			cp.in = featuredInputs;
			cp.out = e.next[i];
